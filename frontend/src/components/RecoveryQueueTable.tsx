...
import { Search, Filter, Cpu, ChevronRight, CheckCircle2, AlertTriangle, XCircle, Clock, MessageSquare, UserCheck, X, Lock } from 'lucide-react';
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'HUMAN_ESCALATION':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-200 dark:border-amber-800 font-bold">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-1.5 text-xs sm:text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          {searchTerm && (
            <button
              onClick={() => setSearchTerm('')}
              aria-label="Clear search"
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-white p-0.5 rounded transition-colors"
            >
              ...
              <tr>
                <td colSpan={8} className="py-12 text-center text-slate-500 dark:text-slate-400">
                  <div className="flex flex-col items-center justify-center space-y-2">
                    ...
                    <p className="font-medium text-slate-700 dark:text-slate-300 text-sm">No payment failures matching your criteria</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 max-w-sm">
                      {searchTerm ? `No results found for "${searchTerm}".` : 'No recovery cases found for the selected status filter.'} Try clearing filters or searching by another ID.
                    </p>
                  </div>
                </td>
              </tr>
              ...
              filteredCases.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-4 text-right space-x-1.5 whitespace-nowrap">
                    {/* Omnichannel Message Preview (Only for customer message actions) */}
                    {onPreviewMessage && ['SEND_PAYMENT_LINK', 'PAYMENT_LINK', 'REMINDER', 'SCHEDULE_FOLLOWUP'].includes(item.recommended_action) && item.status !== 'HUMAN_ESCALATION' && (
                      <button
                        onClick={() => onPreviewMessage(item)}
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/50 dark:hover:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 text-xs font-medium transition-colors border border-emerald-200 dark:border-emerald-800"
                        title="Preview WhatsApp/SMS customer message"
                      >
                    ...
                    {/* Human Escalation Resolve Button */}
                    {onResolveEscalation && (item.status === 'HUMAN_ESCALATION' || item.recommended_action === 'HUMAN_ESCALATION') && (
                      <button
                        onClick={() => onResolveEscalation(item)}
                        disabled={isAuditor}
                        className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow-sm ${
                          isAuditor
                      ...
                      >
                        ...
                        <span>{isAuditor ? 'Auditor' : 'Resolve'}</span>
                      </button>
                    )}
                    ...
                    {item.status === 'LINK_SENT' && onMarkPaid && (
                      <button
                        ...
                        title={isAuditor ? 'Auditor Persona: Read-Only Mode' : 'Simulate customer completing payment link'}
                      >
